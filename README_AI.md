...
> **v1.3.0 γ 交付（全量代码复查修复 27 项）**：
> - 修复 v2 复查清单全部 27 项：中高 1（重连计数成功后不复位）、中 5（村民交易单次、seek_shelter 中途失败无回退、hasEnemyNear 失真、丢弃贵重物保护恒失效、熔炼整叠过量）、低 19（扫描无节流、导航无兜底、空手切换、SWAP 还原、确认窗口、回退半径、waypoints 越界、长超时、快照缓存、Electron 日志、端口探测、槽位范围、默认地址、Locale、off-by-one、accept_items 归属等）、信息 2（丢弃二次确认、冷启动超时建议）。
...
> - 版本号 1.3.0-gamma（npm/桥模组/标签用 `1.3.0-gamma`，文档中文写作 `v1.3.0 γ`）。
...
> - 跨维度：维度切换改为每 tick 检测，持续跟随可跨传送门；空闲在下界/末地时自动返回主世界。
> - 寻路强化：A* 搜索半径 24→40（垂直 6→12，节点上限 6000→12000），能绕过更宽的玩家建筑。
> - 找床睡觉：`sleep_in_bed` 优先用自己的床，找不到再找附近空床（村庄/玩家基地）；夜间主动找床睡觉并设置重生点。
> - 源码注释全部翻译为中文（`start-headless-client.ps1` 保持 ASCII 以兼容 PowerShell 5.1）。
...
3. 确认 `dist/src/index.js`、HeadlessMc jar 和 `.runtime/minecraft/mods/minecraft-ai-fabric-bridge-1.3.0-gamma.jar` 存在。
