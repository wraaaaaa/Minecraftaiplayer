...
const VERIFIED_WORLD_ACTION_RULE = '基础玩法工具：采集 gather_resource、合成 craft_item、放置 place_block、把自身物品交给玩家 drop_item、接收玩家丢出的物品 accept_items、返回住所 return_home。复杂任务可输出 actions 数组（最多 12 步），本地将按顺序逐步能力检查、策略检查和服务器后置条件验证，任一步失败都会停止后续步骤；不得声称动作列表没有这些工具。附近方块信息位于 structuredGameState.blockSurvey，玩家指向方块位于 nearbyPlayers[].lookingAtBlock。不再使用人工 developmentZone；模型根据环境选择意图，Fabric 对每个修改世界的目标执行不可绕过的安全验证。'
