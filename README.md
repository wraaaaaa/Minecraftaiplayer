...
当前默认是 `companion`（陪伴）模式。语言模型负责理解自然语言、聊天与任务级决策；持续跟随、避障、上岸、穿门、动作表情和安全待机在本地客户端逐 Tick 执行，不会每走一步或每放一个方块都请求模型。Bot 没有玩家任务时会回到登记住所或“第一个家”，停止全部普通目标并进入零模型调用待机；有新玩家经过时可先陪走并询问是否需要跟随，遭拒后自动回家。旧 `survival` 自主发育模式仍可手工启用作实验，但不再是默认路线，也不承诺从零自动发展到末地。
...
- 自定义人设、真实回复限频、近距离/语境寻址、显式名称或 `!` 寻址；近距离自然命令无需每次喊 Bot 名。陪伴模式空闲时不调用模型主动发育或闲聊。
...
- 陪伴模式的本地状态机：路过玩家邀请、接受/拒绝、长期跟随、回家与安全待机均不调用模型；收到任务后才启动 Tool Agent。`survival` 模式仍保留旧的 `reach_end` 实验路径，但默认不运行，未完成端到端通关验收。
...
- WebUI 的“陪伴模式（推荐）”是新默认值。没有排队任务、持续跟随或生存紧急情况时，Bot 只执行一次本地回家/安全等待动作，之后不再创建 Tool Agent，也不再请求模型；`chat.proactiveEnabled` 的空闲闲聊默认关闭。
...
- 为配合项目转为陪伴定位，自动采集、挖矿、造房和前往末地不再在默认空闲路径运行。代码仍保留在 `survival` 实验模式和玩家明确任务工具中，便于以后继续开发；没有宣称已经可靠实现全自动生存。
- 本次私有部署实服回归已完成：23 个受管模组同步后客户端通过 EasyAuth 进入主世界，返回“第一个家”半径内，期间本地处理一次真实敌对威胁，随后诊断写入“陪伴模式已进入零 Token 待机”；整个空闲样本模型事件数为 0。现场没有第二名玩家，因此路过邀请/接受拒绝和表情动作以自动测试覆盖，不能冒充多人实服验收。
...
| `autonomy.mode` | `companion` | `companion` 为零 Token 空闲陪伴；`survival` 才启用实验性自主发育循环 |
...
| `autonomy.autoGather/autoCraft/autoBuildShelter` | `true` | 允许玩家明确任务/实验生存模式使用对应能力；陪伴模式不会空闲自动启动 |
