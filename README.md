...
- 跳过模组握手校验：`config/mods.json` 的 `skipHandshakeVerification` 开启后，客户端不再同步服务器模组并以 `-Dfabric.loader.disableHandshake=true` 跳过 Fabric 模组协商，直接进服玩原版内容（是否成功取决于目标服是否在服务端强制校验）。
